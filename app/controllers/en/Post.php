            ...
            if (Token::check(Input::get('token'))) {
                if ($_POST) {
                    ...
                    $validation->check($_POST, [
                        ...
                        'titulo' => [
                            'display' => 'Insira o titulo da notícia',
                            'require' => true
                        ],
                        'text' => [
                            'display' => 'Insira o corpo da notícia',
                            'require' => true
                        ]
                    ]);
                    ...
                    if ($validation->passed()) {
                        ...
                        if (!isset($_FILES['photo']['name'])) {
                            ...
                            //dnd($_FILES['photo']);
                            $img = $Files->SaveAFile($_FILES['photo'], 'posts');
                            //dnd($img);
                            $id_news = $Posts->newPost($_POST, $img);

                            if($id_news > 0){
                                $link = PROOT . "home/postread/" . $id_news;
                                $this->view->displayErrors = $validation->displayOkay("Noticia inserida com sucesso, <a href='$link'>Vizualizar</a>");
                            }else{
